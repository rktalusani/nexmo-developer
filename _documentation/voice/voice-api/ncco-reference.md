...
`split` | Record the sent and received audio in separate channels of a stereo recording—set to `conversation` to enable this.|
No
`channel` | Record up to `32` seprate channels. split `conversation` must also be enabled.| No
